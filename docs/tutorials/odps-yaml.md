...
promises."* That's the **Open Data Product Standard**: this tutorial bundles
the contracts into an ODPS v1.0.0 **data product**.

**Skill showcased**: [`odps-yaml`](../../skills/odps-yaml/). It teaches your
...
- **Input ports** — *expectations*: data the product needs from upstream.
- **Output ports** — *promises*: data the product offers to consumers.
- **Management ports** — *control plane*: endpoints to observe or control the
  product.

Schema, quality, SLA, server bindings? **Not here.** They stay in the
contracts. If you catch yourself writing a column name into an ODPS file,
you've drifted a standard too far up the stack.
...
Create `jaffle_shop.odps.yaml`. Same familiar opening as ODCS — note the
different `kind` and that ODPS v1.0.0 requires only `apiVersion`, `kind`,
`id`, and `status`:
...
- **Input ports have no `description` field.** That's an output-port field —
  put a description on an input port and strict validation fails with
...
RFC 0016 aligned ODPS's team-as-object shape with ODCS v3.1.0):
...
product needs three hops to a queryable table, no tribal knowledge required:

1. **Product** → `outputPorts[0]` promises `orders`, contract
   `8f8e7b5a-…034b`.
2. **Contract** ([`orders.odcs.yaml`](jaffle-shop/contracts/orders.odcs.yaml))
   → schema, quality promises, SLA, and a `duckdb` server binding:
...
to their input, a management port for observability, and shared
support/team — validated against the vendored schema, with the standard's
one sharp edge (no `description` on input ports) experienced first-hand.
