...
| **Product information** | `inputPorts[]` and `outputPorts[]` — the data interfaces. Output ports are required (a product without output is useless); input ports are optional (a pure-source product may have none). Each port has at least `name`, plus optional `version`, `contractId`, `tags`, `customProperties`, `authoritativeDefinitions`. **Output ports only** additionally take `type` and `description`, plus `sbom` and `inputContracts` for provenance — the JSON Schema rejects `description`/`type` on input ports. | `outputPorts` recommended |
...
- **Putting `description` (or `type`) on an input port.** Those are output-port fields; the v1.0.0 JSON Schema rejects them on `inputPorts[]` entries (`Additional properties are not allowed`). An input port is described by the contract it references.
